...
class StrapiAgent:
    def __init__(self, strapi_url: str, headers: object, medel_chat: LanguageModelLike):
...
""",
        ...
        self.site_data_extractor = site_data_prompt | self.llm.with_structured_output(SiteData)
        
        
        self.agent_executor = create_react_agent(self.llm, self.tools) #, state_modifier=self.executor_state_modifier)
        self.agent_executor.step_timeout = 15
...
You are tasked with executing step {1}, {task}.
    """
        logger.info(f"Agent executor prompt: {task_formatted}")
        
        instructions = """
You are a helpful assistant that helps creating content for a company website. You are provided with a company profile description, and a step to execute which is part of a plan.
...
Step exectution Guidelines:
- Execute only the step provided in the task description, not the whole plan. After max 3 attempts to exectute the step, return.
- The context of the generated website text content must always be the provided company profile description and the name of the page (where available). For example, a text in the stage component on the home page should highlight the key aspects of the company.
- Always review the text parameters in input, descriptions and text content should ALWAYS be at least 100 words long, and elaborated on the provided company profile description.
- Where provided review the input parameters related to SEO (search engine optimization), they should be relevant to the company profile description.
...
"""
        ...
        # agent_response = self.agent_executor.invoke({"input": task_formatted, "company_profile": state["company_profile"]})
        return {
...
- Information about services or products offered""")        
    ...
    def replan_step(self, state: AgentState):
        logger.info(f"Executing step: {inspect.currentframe().f_code.co_name}")
        logger.debug(f"Agent state (before replanning): {state}")
        
        output = self.replanner.invoke(state)
        if isinstance(output.action, Response):
            logger.debug(f"No more steps needed. Returning response: {output.action.response}")
    ...
    def generate_company_profile_description(self):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "Generate random dummy company profile descriptions including the company name, industry, and information about services or products offered. Pick the industry randomly, e.g. fashion, tech, food, travel, wellness, etc. "),
